...
def get_verification_scores(veri_test):
    ...
    for i, line in enumerate(veri_test):
        ...
        lab_pair = int(line.strip().split(" ")[0])
        enrol_id = line.split(" ")[1].rstrip().split('/')[-1].split(".")[0].strip()
        test_id = line.split(" ")[2].rstrip().split('/')[-1].split(".")[0].strip()
...
def dataio_prep(params):
    ...
    enrol_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
        csv_path=params["enrol_data"], replacements={"data_root": enroll_dir},
    )
    enrol_data = enrol_data.filtered_sorted(sort_key="duration")

    # Test data
    test_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
        csv_path=params["test_data"], replacements={"data_root": test_dir},
    )
